...
function json() {
  return gulp.src('./src/js/*.json').pipe(gulp.dest('./dist/js')).pipe(browserSync.stream());
}

function libs() {
  return gulp
    .src(['./node_modules/slick-carousel/slick/slick.min.js', './node_modules/jquery/dist/jquery.min.js'])
...
let build = gulp.parallel(html, styles, images, js, libs, webfonts, json, favicon);
